...
class Login extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    this.setState({
      ...
      try {
        // here we use Login.service instead this.service because it is static
        const result = await Login.service.login(credentials);
        if (!result.success) {
          const errors = Object.values(result.error).join(' ');
...
const LoginWithContext = (props) => {
  return (
    // consumer will receive a function and its props will be whatever is provided from provider
    <UserConsumer>
